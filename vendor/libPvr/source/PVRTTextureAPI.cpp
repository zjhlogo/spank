										...
										CPVRTMap<unsigned int, CPVRTMap<unsigned int, MetaDataBlock> > *pMetaData)
{
	...
	if (eTextureFormat==0 && eTextureType==0 && eTextureInternalFormat!=0)
	{
		if (eTextureInternalFormat>=GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG && eTextureInternalFormat<=GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG)
		{
			...
			else
			{
				...
				if(bAllowDecompress)
				{
					...
					{
						LOGE("PVRTTextureLoadFromPointer error: Unable to allocate memory to decompress texture.");
						return PVR_FAIL;
					}
		...
		else if (eTextureInternalFormat==GL_ETC1_RGB8_OES)
		{
			...
			else
			{
				...
				{
					//Output a warning.
					LOGE("PVRTTextureLoadFromPointer warning: ETC not supported. Converting to RGBA8888 instead.");
				...
				{
					LOGE("PVRTTextureLoadFromPointer error: ETC not supported.");
					return PVR_FAIL;
				}
			}
		}
	...
	{
		LOGE("PVRTTextureLoadFromPointer failed: pixel type not supported.");
		return PVR_FAIL;
	}
	...
	if (bIsLegacyPVR)
	{
		...
		for (PVRTuint32 uiFace=0; uiFace<psTempHeader->u32NumFaces; ++uiFace)
		{
			...
			if(glGetError())
			{
				...
				LOGE("PVRTTextureLoadFromPointer failed: glTexImage2D() failed.");
				return PVR_FAIL;
			}
		}
	}
	...
	if(glGetError())
	{
		LOGE("PVRTTextureLoadFromPointer failed: glTexParameter() failed.");
		return PVR_FAIL;
	}
									...
									CPVRTMap<unsigned int, CPVRTMap<unsigned int, MetaDataBlock> > *pMetaData)
{
	...
	if (*(PVRTuint32*)buffer.data() != PVRTEX_CURR_IDENT && *(PVRTuint32*)buffer.data() != PVRTEX_CURR_IDENT_REV)
	{
		//Swap the header bytes if necessary.
		if(!PVRTIsLittleEndian())
		{
			bSwapDataEndianness=true;
			PVRTuint32 u32HeaderSize = PVRTByteSwap32(*(PVRTuint32*)buffer.data());

			for (PVRTuint32 i=0; i<u32HeaderSize; ++i)
			{
				PVRTByteSwap((PVRTuint8*)(((PVRTuint32*)buffer.data()) + i), sizeof(PVRTuint32));
			}
		}

		//Get a pointer to the header.
		PVR_Texture_Header* sLegacyTextureHeader = (PVR_Texture_Header*)buffer.data();
	...
	else if (*(PVRTuint32*)buffer.data() == PVRTEX_CURR_IDENT_REV)
	{
		//Setup the texture header
		sTextureHeader = *(PVRTextureHeaderV3*)buffer.data();

		bSwapDataEndianness=true;
		PVRTextureHeaderV3* pTextureHeader = (PVRTextureHeaderV3*)buffer.data();
	...
	if(bSwapDataEndianness)
	{
		...
		{
			//Get the texture data.
			PVRTuint8* pu8OrigData = ((PVRTuint8*)buffer.data() + u32HeaderSize);
